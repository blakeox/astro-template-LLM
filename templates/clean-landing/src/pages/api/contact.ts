...
app.post("/", async (c) => {
  ...
  if (!v.ok) {
    const errorResponse = v as { ok: false; code: number; err: string; details?: string[] };
    return c.json({ error: errorResponse.err, details: errorResponse.details || [] }, errorResponse.code);
  }
