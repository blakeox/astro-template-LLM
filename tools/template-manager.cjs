...
async function validateTemplate(templateId) {
  ...
  try {
    ...
    if (fs.existsSync(manifestPath)) {
      ...
      const requiredFields = ['id', 'name', 'description', 'version', 'license', 'author'];
      ...
      if (manifest.id !== templateId) {
        errors.push(`Manifest ID "${manifest.id}" does not match template directory "${templateId}"`);
      }
    }
  } catch (error) {
...
async function updateRegistry() {
  ...
  for (const templateId of templateDirs) {
    ...
    try {
      ...
      if (manifest.framework) {
        stats.frameworks[manifest.framework] = (stats.frameworks[manifest.framework] || 0) + 1;
      }
      
      if (manifest.styling) {
        stats.styling[manifest.styling] = (stats.styling[manifest.styling] || 0) + 1;
      }

      if (manifest.deployment) {
        for (const target of manifest.deployment) {
          stats.deployment_targets[target] = (stats.deployment_targets[target] || 0) + 1;
        }
      }
  ...
  const registry = {
    ...
    description: "A curated collection of Astro templates for rapid website development",
    version: "1.0.0",
    templates,
    stats,
    api_version: "1.0",
...
async function main() {
  try {
    switch (command) {
      case 'validate':
        if (!templateId) {
          console.error('Usage: node template-manager.js validate <template-id>');
          process.exit(1);
        }
