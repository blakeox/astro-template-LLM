 ...
 */
function sanitizeTextContent(config: SiteConfig): {
	changed: boolean;
	warnings: string[];
} {
	const warnings: string[] = [];
	...
	function cleanText(text: string, fieldName: string): string {
		...
		for (const pattern of dangerousPatterns) {
			...
			if (matches) {
				...
				warnings.push(
					`Removed dangerous content from ${fieldName}: ${matches[0].substring(0, 50)}...`,
				);
				changed = true;
			}
		}
...
function sanitizeUrls(
	...
	if (hero.cta?.primary?.href) {
		hero.cta.primary.href = validateUrl(
			hero.cta.primary.href,
			"hero.cta.primary.href",
		);
	}
	if (hero.cta?.secondary?.href) {
		hero.cta.secondary.href = validateUrl(
 ...
 */
function enforceCharacterLimits(config: SiteConfig): {
	changed: boolean;
	warnings: string[];
} {
	const warnings: string[] = [];
	...
	function truncateField(
		...
		if (text.length > maxLength) {
			warnings.push(
				`Truncated ${fieldName} from ${text.length} to ${maxLength} characters`,
			);
			changed = true;
	...
	if (hero.cta?.primary?.text) {
		hero.cta.primary.text = truncateField(
	...
	if (hero.cta?.secondary?.text) {
		hero.cta.secondary.text = truncateField(
	...
	for (let i = 0; i < config.pages.home.features.length; i++) {
		...
		feature.description = truncateField(
			feature.description,
			120,
			`features[${i}].description`,
		);
	...
	if (config.pages.about?.blurb) {
		config.pages.about.blurb = truncateField(
			config.pages.about.blurb,
			500,
			"about.blurb",
		);
	}
 ...
 */
export function validateProfessionalTone(config: SiteConfig): {
	valid: boolean;
	warnings: string[];
} {
	const warnings: string[] = [];
	...
	for (const pattern of unprofessionalPatterns) {
		...
		if (matches) {
			warnings.push(
				`Consider revising potentially unprofessional language: ${matches[0]}`,
			);
		}
	}
