...
export async function verifyTurnstile({
  EDGE_STORE, secret, token, ip, salt
}: {
  EDGE_STORE: KVNamespace; secret: string; token: string; ip?: string; salt: string;
}) {
  ...
  if (!body.success) {
    ...
    await EDGE_STORE.put(k, String(current + 1), { expirationTtl: 3 * 60 * 60 });
    return { ok: false, code: 403, err: "turnstile failed", details: body["error-codes"] || [] };
  }
