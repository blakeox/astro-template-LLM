...
async function validateSiteConfig(configPath: string): Promise<{ valid: boolean; config?: SiteConfig; errors?: string[] }> {
  try {
...
async function generateFromPrompt(prompt: string): Promise<SiteConfig> {
  ...
  const siteDescription = extractSiteDescription(prompt, siteName) || "Professional website generated from prompt";
...
function generateFeatures(prompt: string): any[] {
  if (prompt.toLowerCase().includes('studio') || prompt.toLowerCase().includes('design')) {
    return [
      {
        icon: "🎨",
        title: "Brand Design",
  ...
  if (prompt.toLowerCase().includes('agency') || prompt.toLowerCase().includes('development')) {
    return [
      {
        icon: "⚡",
  ...
  if (options.prompt) {
    try {
      ...
      if (!validationResult.success) {
        result.valid = false;
        result.errors = validationResult.error.errors.map(err => `${err.path.join('.')}: ${err.message}`);
